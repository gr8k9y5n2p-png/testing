...
- **Account store (required for Production login):** set Upstash Redis REST URL + token (`UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN`, or leftover `KV_REST_API_*`). Private Blob is the fallback only when those Redis vars are unset — this repo versions Blob writes so a stale put cannot clobber a newer document. Do not use `/tmp` / `AFTERTAX_ACCOUNTS_PATH` on Vercel Functions — that is why login still said the password was wrong after #277.
...
- Soft wall is **on** (5 searches / 3 compare reports / 3 portfolio reviews). Over-limit modules blur with an Unlock CTA; the search box, Request a fund, and ticker slots stay usable. The **Lists** tab (`/lists`) is fully behind the same Unlock Access wall for signed-out / freemium / not-entitled users — blur + centered Unlock module with a swappable Lists preview. Homepage **Highlights** and **Upcoming / Announced** use that same entitlement wall (no free quota). Paid History stays on the Search quota wall. Subscribers (or `NEXT_PUBLIC_FREEMIUM_DISABLED=true`) see the full Lists page. `GET /api/lists` and list Save/Open return 403 when not entitled.
...
3. After 5 searches / 3 compare reports / 3 portfolio reviews → soft blur + Unlock CTA (`$39 / user / month`). Homepage search box and Request a fund stay usable. Lists, homepage Highlights, and Upcoming / Announced have no free quota — Unlock Access unless subscribed. Paid History stays on the Search quota wall.
...
| Value | Behavior |
| --- | --- |
| unset / `false` / `0` / `off` | 5 searches / 3 compares / 3 portfolio reviews, then blur + CTA |
...
**Production setup (do this before relying on login):** Prefer **Upstash Redis** (`UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN`, aliases `KV_REST_API_URL` / `KV_REST_API_TOKEN`). Redis is the concurrency-safe default: one REST GET plus an `EVAL` compare-and-swap so a stale isolate cannot overwrite a newer document. Those vars are used automatically when present. **Blob only if Redis is unset** — and only because this repo writes `aftertax/accounts.vN.json` with `allowOverwrite: false` and retries on conflict. A stale `put` of `accounts.json` used to drop other emails (signup 201, then “Email or password is wrong”). Do not point Production at Blob without that versioned CAS. Vercel → Storage → **Blob** (Private) injects `BLOB_READ_WRITE_TOKEN` / `BLOB_STORE_ID`. Blob reads `list` the prefix (highest generation wins — `get(pathname)` can 404 after `put`), then fetch that private URL with the Blob token. Isolate last-URL reuse expires in 2.5s. Fallback `get(url, { useCache: false })` only if fetch misses. Each read or write has a **2.5s wall-clock budget** (not stacked 8s calls). The account store keeps a **2.5s hydrate cache** so `findById` + `updateBilling` during Checkout do not re-read Blob; writes still CAS against the last durable payload and retry. Signup does not return 201 until a fresh durable read-back contains that email. A missing blob is an empty array. Timeouts throw. Hashes stay scrypt.
