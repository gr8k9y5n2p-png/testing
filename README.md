...
- **Portfolio comparison** on `/portfolio` (and `/portfolio-compare`): Current vs Proposed Allocation, GTM history-covered smoke books (Current AGTHX / DODIX / AMCAP / DODGX and Proposed AMCPX / CGHM / AGTHX / AMCAP at 25% each) + $1M + state 0.05, per-column tax-impact bars, compact upcoming-distributions table, tax drag %, more/less tax Δ. The Portfolio tab does not show Growth of $X, fund-search hero, dollar illustration, FundCompareRail, or highlights. Single snapshot only — no YoY bars / no `periods[]`. Export calls `exportToPdf` (freemium gate stubbed).
...
- `POST /illustrate/portfolio/compare` — Current vs Proposed Allocation. Body: `current` / `proposed` `{ label?, holdings[], book_dollars }` with `ticker` + `weight_pct` (or `holding_dollars`). Deltas are **proposed − current** (`deltas.estimated_tax`, `effective_tax_on_holding`). v1 is a single snapshot — **do not send `periods[]`**. Bar charts + Upcoming heat tables use `holdings[].upcoming` `{ distribution_dollars, estimated_tax, as_of, publication_stage, record_date, ex_date, payable_date }` (`null` = no unpaid announced; do not derive). Paid History prefers additive `holdings[].paid_history[]` (same row shape; Data ships this on PR #2). Until that field is live, the table falls back to `illustration.components` (else `distributions`) whose record/ex/payable are past or `publication_stage` is `paid`/`final`. Do not invent dates. Each column shows **Total tax impact** = Σ `upcoming.estimated_tax` above the chart. Current/Proposed tax drag cards read `totals.effective_tax_on_holding` / `totals.estimated_tax` and Δ is `deltas.estimated_tax` (proposed − current) — independent of empty Upcoming. Older payloads without `upcoming` still fall back to `illustration` + `publication_stage_used`. If compare is 404/down, the client double-calls `POST /illustrate/portfolio` and synthesizes deltas; localhost falls back to the sketch fixture. Print/PDF: `exportToPdf(toPortfolioCompareExportModel(result, bookDollars))` — Website wires the button + freemium gate. Product view: `/portfolio`. Standalone demo: `/portfolio-compare`. Import `PortfolioCompare`, `exportToPdf`, and `toPortfolioCompareExportModel` from `@/components/illustrate`.
