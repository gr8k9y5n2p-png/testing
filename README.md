...
**Fund comparison** is a dedicated **Compare** tab at [http://localhost:3000/compare](http://localhost:3000/compare) (`/?tab=compare` and `/#fund-compare` redirect there). **Fund Comparison** opens that view — six empty ticker slots, then `GrowthAndTaxDragModule`, a calendar-year tax/distribution table, then Upcoming / announced. Standalone tax-delta demo: [http://localhost:3000/fund-compare](http://localhost:3000/fund-compare). Import `CompareWorkspace` from `@/components/illustrate`.
...
- **Compare** on `/compare`: six empty ticker slots (user-added; **one ticker is enough** — empty slots ignored). A compact **delta strip** stub (Total tax $ Δ on $10k, Annualized tax-drag Δ %, Distribution $ Δ on window, Upcoming taxable / Undisclosed) mounts without blocking the page. Filled tickers then flow through `GrowthAndTaxDragModule`, a ticker × calendar-year tax $ / dist $ table from live `POST /illustrate/compare` `mode: "yoy"` (unmatched = N/A, never fake $0), then Upcoming / announced (unpaid announced only; empty = Undisclosed / Not available — never invented from paid history). `?left=` / `?right=` may prefill the first slots; a plain `/compare` visit starts empty. The Compare tab does not show FundTaxDeltaCompare as the main layout, the fund-search hero, dollar illustration, portfolio books, or highlights. Historical tax drag and Upcoming stay separate. The `/fund-compare` demo still mounts the reusable `FundTaxDeltaCompare` card.
