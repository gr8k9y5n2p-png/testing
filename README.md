...
**Official all-events leftover (after `#186` MFS mid-year tip, in-book only):** Exclusive slice: published **midyear / quarterly / secondary** events the prior December-only extracts omitted, with correct types (ordinary income, STCG, LTCG, ROC, special). Existing tickers only in American Funds / Capital Group, Vanguard, T. Rowe, BlackRock / iShares (Fidelity already multi-event; JPM / PIMCO / Franklin / Invesco leftover midyear unpublished). **Adopts already-merged `#186`** MFS mid-year fills (`paid_midyear.html` + Type of Earnings parser) — does not overwrite or drop them. Fixture digest on the `#186` tip was **3,540** (`funds_with_5y` 3,540 / MF 2,790 / ETF 750). After this slice the pin is recomputed additively on that tip (ETF 5y unchanged at **750**; no new identities). Live lock stays **`funds_total=10056`**. Calendar year stays **ex_date, else payable_date, else as_of**. Additive upserts only — never collapse a multi-event year onto one YE `as_of`. **Densified (22,164 typed non-MFS events):**
...
| Virtus | 2024 19(a) + 2025 paid + 2026 June full listed estimate | No public filled ICI. 2026 June PDF lists 4 funds (STVTX LT $0.1767); 2025 calyr paid Dec (STVTX LT $0.427834); 2024 19(a) income + combined CG $1.907616. Parallel J leftover re-probe: 2021–2024 calendar filenames still serve the 2025 book (identical hash). Product-page Distribution History is JS. Leftover 2021–2024 stay unmatched. |
...
| 29 | `virtus` | Virtus | implemented | PDF | 2026 June estimate + 2025 calyr paid + 2024 19(a) (STVTX). Parallel J leftover: 2021–2024 calendar filenames alias the 2025 book. |
