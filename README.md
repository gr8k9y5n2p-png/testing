...
- **Portfolio comparison** on the homepage (and `/portfolio-compare`): Current vs Proposed Allocation, GTM history-covered smoke books (Current AGTHX / DODIX / AMCAP / DODGX and Proposed AMCPX / CGHM / AGTHX / AMCAP at 25% each) + $1M + state 0.05, per-column tax-impact bars, compact upcoming-distributions table, tax drag %, more/less tax Δ. Single snapshot only — no YoY bars / no `periods[]`. Export calls `exportToPdf` (freemium gate stubbed).
