...
**Production setup (do this before relying on login):** Prefer **Upstash Redis** (`UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN`) for this single JSON document — one REST GET/SET, no pathname 404s. Redis is used automatically if those REST vars are already present. Otherwise Vercel → Storage → **Blob** (Private) → connect → redeploy so `BLOB_READ_WRITE_TOKEN` / `BLOB_STORE_ID` are present. Blob reads `list` the prefix (newest pathname wins — `get(pathname)` can 404 after `put`), then fetch that private URL with the Blob token. A successful `put` caches that URL for the next read on the same isolate. Fallback `get(url, { useCache: false })` only if fetch misses. Each read or write has a **2.5s wall-clock budget** (not stacked 8s calls). The account store also keeps a **2.5s hydrate cache** so `findById` + `updateBilling` during Checkout do not re-read Blob. A missing blob is an empty array. Timeouts throw. Hashes stay scrypt.
