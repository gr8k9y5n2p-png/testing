...
On the American Funds fixtures: $1.25M covered / $150k uncovered → `coverage_pct` ≈ 89.3%. AMCAP uses the latest preliminary (3–5% NAV → $40,000 / $10,000 tax at 20%+5%). Each covered holding with distribution dollars also gets `upcoming: {distribution_dollars, estimated_tax, as_of, publication_stage, record_date, ex_date, payable_date}` from that chosen illustration (`null` on gaps or when dollars are zero — e.g. CGHM without NAV). Calendar dates are copied from the source row when published and left `null` when unknown so the UI can show —. `XYZAX` is a gap.
