...
- One-screen landing: hero (taxable impact in dollars) + **Search a fund** as the primary action. Header tabs: **Search** (`/`) and **Portfolio** (`/portfolio`). CTA order when present: **Search a fund** → **Fund Comparison** → **Import a portfolio**. Import opens the Portfolio tab, not a homepage scroll target and not the paywall.
...
1. Land → search a fund (primary). Import a portfolio opens the Portfolio tab (`/portfolio`).
