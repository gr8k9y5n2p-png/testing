...
- **Compare** on `/compare`: six empty ticker slots (user-added; **one ticker is enough** — empty slots ignored). Filled tickers flow through the same `GrowthAndTaxDragModule` as Search, then a ticker × calendar-year tax $ / dist $ table from live `POST /illustrate/compare` `mode: "yoy"` (unmatched = N/A, never fake $0), then Upcoming / announced for those tickers (unpaid announced only; empty = Undisclosed / Not available — never invented from paid history). `?left=` / `?right=` may prefill the first slots; a plain `/compare` visit starts empty. The Compare tab does not show FundTaxDeltaCompare as the main layout, the fund-search hero, dollar illustration, portfolio books, or highlights. Historical tax drag and Upcoming stay separate. The `/fund-compare` demo still mounts the reusable `FundTaxDeltaCompare` card.
